...
<main>
  ...
  <section id="content1">
		<form method="POST" action="saveWebsite.php" enctype="multipart/form-data">
			...
			<button class="btn btn-default cmsButtons" type="submit" formaction="#">Check</button>
			<input class="btn btn-default cmsButtons" type="submit" value="Opslaan" />
		<?php
		if($existingWebsite) {
			echo "<button class='btn btn-default cmsButtons' formaction=\"deleteWebsite.php?websiteID=" . $result["websiteID"] . "\">Delete website</button><br />";
		}
  ...
  <section id="content4">
	...
	while($t = $query3->fetch()) {
		echo "<tr>";
		echo "<td><img src='../../img/testafbeeldingen/2.jpg'></td>";
		echo "<td>" . $t["description"] . "</td>";
		echo "<td><textarea class='textareaWebsiteText' name=\"" . $t["textName"] . "\" placeholder=\"Voorbeeld: " . $t["exampleText"] . "\">" . $t["text"] . "</textarea></td>";
		echo "</tr>";
	}
	$query3 = null;
	?>
